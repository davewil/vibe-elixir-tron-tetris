defmodule TronTetrisWeb.TetrisLive do
  ...
  @impl true
  def mount(params, _session, socket) do    # Check if player ID is in params or generate a new one
    ...
    player = case Integer.parse(to_string(player_id)) do
      {_int_id, ""} -> 
  ...
  def handle_info({:menu_action, action}, socket) do
    case action do
      ...
      {:toggle_sound, sound_state} ->
        ...
        {:noreply, socket |> assign(:sound, sound_state) |> push_event("toggle_sound", %{enabled: sound_state})}
  ...
  defp handle_load_game(%{assigns: %{player: nil}} = socket) do
    ...
    case TronTetris.Game.ScoreService.get_last_saved_game(player.id) do
      {:ok, board} ->
        ...
        {:noreply, socket |> assign(:board, loaded_board) |> assign(:paused, true) |> put_flash(:info, "Game loaded successfully")}
