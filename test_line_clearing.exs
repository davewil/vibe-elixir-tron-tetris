...
# Test script to verify line clearing works
Mix.install([
  {:tron_tetris, path: "."}
])
...
# Create a board with a full line except for one block
board = Board.new()
IO.puts("Initial board: #{inspect(board.landed_tetrominos)}")
...
IO.puts("Board with 9 blocks in bottom row: #{inspect(board.landed_tetrominos)}")
